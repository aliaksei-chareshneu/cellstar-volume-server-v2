...
from preprocessor_old.params_for_storing_db import CHUNKING_MODES, COMPRESSORS
from preprocessor_old.src.preprocessors.implementations.sff.preprocessor.constants import (
    APPLICATION_SPECIFIC_SEGMENTATION_EXTENSIONS,
    DEFAULT_DB_PATH,
)
from preprocessor_old.src.preprocessors.implementations.sff.preprocessor.constants import (
...
from preprocessor_old.src.preprocessors.implementations.sff.preprocessor.constants import (
    PARAMETRIZED_DBS_INPUT_PARAMS_FILEPATH,
    RAW_INPUT_FILES_DIR,
    TEMP_ZARR_HIERARCHY_STORAGE_PATH,
)
from preprocessor_old.src.preprocessors.implementations.sff.preprocessor.sff_preprocessor import (
    SFFPreprocessor,
)
from preprocessor_old.src.service.implementations.preprocessor_service import (
    PreprocessorService,
)
from preprocessor_old.src.tools.convert_app_specific_segm_to_sff.convert_app_specific_segm_to_sff import (
    convert_app_specific_segm_to_sff,
)
from preprocessor_old.src.tools.remove_files_or_folders_by_pattern.remove_files_or_folders_by_pattern import (
    remove_files_or_folders_by_pattern,
)
from preprocessor_old.src.tools.write_dict_to_file.write_dict_to_txt import (
    write_dict_to_txt,
)
        ...
        for item in content:
            if item.is_file():
                if item.suffix in APPLICATION_SPECIFIC_SEGMENTATION_EXTENSIONS:
                    sff_segmentation_hff_file = convert_app_specific_segm_to_sff(
                        input_file=item
                    )
    ...
    if hardcoded:
        dummy_dict = {
            "emdb": [
                {
                    "id": "emd-1832",
                    "volume_file_path": Path(__file__)
                    / RAW_INPUT_FILES_DIR
                    / "emdb"
                    / "emd-1832"
                    / "EMD-1832.map",
                    "segmentation_file_path": Path(__file__)
                    / RAW_INPUT_FILES_DIR
                    / "emdb"
                    / "emd-1832"
                    / "emd_1832.hff",
        ...
        for dir_path in raw_input_files_dir.iterdir():
            if dir_path.is_dir():
                ...
                for subdir_path in dir_path.iterdir():
                    single_entry_dict = obtain_paths_to_single_entry_files(
                        input_files_dir=subdir_path
                    )
                    d[source_db].append(single_entry_dict)
    return d


async def preprocess_everything(
    db: VolumeServerDB,
    raw_input_files_dir: Path,
    params_for_storing: dict,
    temp_zarr_hierarchy_storage_path: Path,
) -> None:
    preprocessor_service = PreprocessorService(
        [SFFPreprocessor(temp_zarr_hierarchy_storage_path)]
    )
    ...
    for source_name, source_entries in files_dict.items():
        for entry in source_entries:
            ...
            segm_file_type = preprocessor_service.get_raw_file_type(
                entry["segmentation_file_path"]
            )
            ...
            await db.store(
                ...
                key=entry["id"],
                temp_store_path=processed_data_temp_path,
            )


async def preprocess_single_entry(
    db: VolumeServerDB,
    ...
    segm_file_type = preprocessor_service.get_raw_file_type(
        files_dict["segmentation_file_path"]
    )
    file_preprocessor = preprocessor_service.get_preprocessor(segm_file_type)
    ...
    await db.store(
        namespace=source_db, key=entry_id, temp_store_path=processed_data_temp_path
    )
...
async def check_read_slice(db: VolumeServerDB):
    ...
    with db.read(namespace="emdb", key="emd-99999") as reader:
        slice_emd_99999 = await reader.read_slice(
            lattice_id=0, down_sampling_ratio=1, box=box
        )
    ...
    print(
        f"volume slice_emd_1832 shape: {emd_1832_volume_slice.shape}, dtype: {emd_1832_volume_slice.dtype}"
    )
    print(emd_1832_volume_slice)
    print(
        f"segmentation slice_emd_1832 shape: {emd_1832_segm_slice.shape}, dtype: {emd_1832_segm_slice.dtype}"
    )
    print(emd_1832_segm_slice)
    print()
    print(
        f"volume slice_emd_99999 shape: {emd_99999_volume_slice.shape}, dtype: {emd_99999_volume_slice.dtype}"
    )
    print(emd_99999_volume_slice)
    # print(slice)
    return {"slice_emd_1832": slice_emd_1832, "slice_emd_99999": slice_emd_99999}


async def check_read_meshes(db: VolumeServerDB):
    with db.read(namespace="empiar", key="empiar-10070") as reader:
        read_meshes_list = await reader.read_meshes(segment_id=15, detail_lvl=3)
...
def create_dict_of_input_params_for_storing(
    chunking_mode: list, compressors: list
) -> dict:
    i = 1
    d = {}
...
def _compose_voxel_sizes_in_downsamplings_dict(ome_zarr_root):
    ...
    labels_datasets_meta = ome_zarr_root["labels"][0].attrs["multiscales"][0][
        "datasets"
    ]
    ...
    for index, level in enumerate(datasets_meta):
        ...
        assert (
            scale_arr
            == labels_datasets_meta[index]["coordinateTransformations"][0]["scale"]
        )
        # no channel, *micrometers to angstroms
        scale_arr = scale_arr[1:]
        scale_arr = [i * 10000 for i in scale_arr]
        # x and z swapped
        d[level["path"]] = (scale_arr[2], scale_arr[1], scale_arr[0])
...
def get_channel_annotations(ome_zarr_attrs, volume_channel_annotations_dict):
    for channel_id, channel in enumerate(ome_zarr_attrs["omero"]["channels"]):
        volume_channel_annotations_dict["colors"][
            str(channel_id)
        ] = _convert_hex_to_rgba_fractional(channel["color"])
...
def extract_ome_zarr_metadata(
    our_zarr_structure: zarr.hierarchy.group,
    source_db_id: str,
    source_db_name: str,
    ome_zarr_root: zarr.hierarchy.group,
) -> Metadata:
    ...
    metadata_dict = {
        "entry_id": {"source_db_name": source_db_name, "source_db_id": source_db_id},
        "volumes": {
...
def extract_ome_zarr_annotations(
    our_zarr_structure, ome_zarr_root, source_db_id: str, source_db_name: str
):
    d = {
        "entry_id": {"source_db_name": source_db_name, "source_db_id": source_db_id},
        "lattices": [],
        "details": None,
        "volume_channels_annotations": {"colors": {}, "labels": {}},
    }
    ...
    for label_gr_name, label_gr in ome_zarr_root.labels.groups():
        ...
        # collect segments to segment_list
        ...
        for ind_label_meta in labels_metadata_list:
            ...
            segment_list = lattice_info["segment_list"]
            segment_list.append(
                {
                    "id": int(label_value),
        ...
        # collect time frame information
        lattice_id = label_gr_name
        lattice_id_group = our_zarr_structure[SEGMENTATION_DATA_GROUPNAME][lattice_id]
        lattice_info["time_frames"][lattice_id] = {}
        for res, res_gr in lattice_id_group.groups():
            lattice_info["time_frames"][lattice_id][res] = {}
            for time, time_gr in res_gr.groups():
                lattice_info["time_frames"][lattice_id][res][time] = {}
                for channel, channel_gr in time_gr.groups():
                    segment_ids = np.unique(channel_gr.grid[...])
                    segment_ids = segment_ids[segment_ids != 0].tolist()
                    # print(segment_ids)
                    lattice_info["time_frames"][lattice_id][res][time][
                        channel
                    ] = segment_ids

        d["lattices"].append(lattice_info)
        # segment_list.append(
        #     {
        #         "id": int(label_value),
        #         "biological_annotation": {
...
def process_ome_zarr(
    ome_zarr_path, temp_zarr_hierarchy_storage_path, source_db_id, source_db_name
):
    ...
    our_zarr_structure = zarr.open_group(
        temp_zarr_hierarchy_storage_path / entry_id, mode="w"
    )
    ...
    SFFPreprocessor.temp_save_metadata(
        grid_metadata, GRID_METADATA_FILENAME, our_zarr_structure_path
    )
    SFFPreprocessor.temp_save_metadata(
        annotation_metadata, ANNOTATION_METADATA_FILENAME, our_zarr_structure_path
    )
...
async def store_ome_zarr_structure(
    db_path, temp_ome_zarr_structure, source_db, entry_id
):
    new_db_path = Path(db_path)
    if new_db_path.is_dir() == False:
        new_db_path.mkdir()
    ...
    await db.store(
        namespace=source_db,
        key=entry_id,
        temp_store_path=Path(temp_ome_zarr_structure.store.path),
    )
...
async def main():
    ...
    if args.temp_zarr_hierarchy_storage_path:
        ...
        temp_zarr_hierarchy_storage_path = (
            TEMP_ZARR_HIERARCHY_STORAGE_PATH / args.db_path.name
        )
    ...
    if args.create_parametrized_dbs:
        ...
        storing_params_dict = create_dict_of_input_params_for_storing(
            chunking_mode=CHUNKING_MODES, compressors=COMPRESSORS
        )
        ...
        if args.quantize_volume_data_dtype_str:
            params_for_storing[
                "quantize_dtype_str"
            ] = args.quantize_volume_data_dtype_str

        if args.single_entry:
            if (
                args.entry_id
                and args.source_db
                and args.source_db_id
                and args.source_db_name
            ):
                if args.ome_zarr_path:
                    ...
                    await store_ome_zarr_structure(
                        args.db_path,
                        zarr_structure_to_be_saved,
                        args.source_db,
                        args.entry_id,
                    )
                    print(1)
                ...
                raise ValueError(
                    "args.entry_id and args.source_db and args.source_db_id and args.source_db_name are required for single entry mode"
                )
        else:
            await create_db(
                Path(args.db_path),
...
def parse_script_args():
    ...
    parser.add_argument(
        "--db_path", type=Path, default=DEFAULT_DB_PATH, help="path to db folder"
    )
    parser.add_argument(
        "--raw_input_files_dir_path",
        type=Path,
        default=RAW_INPUT_FILES_DIR,
        help="path to directory with input files (maps and sff)",
    )
    parser.add_argument("--create_parametrized_dbs", action="store_true")
    parser.add_argument(
        "--quantize_volume_data_dtype_str", action="store", choices=["u1", "u2"]
    )
    parser.add_argument(
        "--single_entry",
        type=Path,
        help="path to folder with MAP and segmentation files",
    )
    parser.add_argument(
        "--entry_id", type=str, help="entry id to be used as DB folder name"
    )
    parser.add_argument(
        "--source_db",
        type=str,
        help="source database name to be used as DB folder name",
    )
    parser.add_argument(
        "--force_volume_dtype", type=str, help="dtype of volume data to be used"
    )
    parser.add_argument(
        "--temp_zarr_hierarchy_storage_path",
        type=Path,
    ...
    parser.add_argument(
        "--source_db_id", type=str, help="actual source db id for metadata"
    )
    parser.add_argument(
        "--source_db_name", type=str, help="actual source db name for metadata"
    )

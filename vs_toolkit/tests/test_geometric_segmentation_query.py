...
METADATA_OUTPUT_FILE_PATH = Path('query_app/tests/test_output/geometric_segmentation_metadata.json')
...
async def test_geometric_segmentation_query():
    metadata_commands_lst = [
        ...
        "metadata",
        "--entry-id", "pdbe-1.rec-geometric_segmentation",
        "--source-db", "pdbe",
                # "--time", "0",
                # "--channel-id", "0",
    ...
    segmentation_id = read_json_of_metadata.get("geometric_segmentation").get("segmentation_ids")[0]
    time_info_dict: dict = read_json_of_metadata.get("geometric_segmentation").get("time_info")
    ...
    commands_lst = [
        ...
        "--segmentation-id", str(segmentation_id),
    ]
    subprocess.run(
        commands_lst
    )

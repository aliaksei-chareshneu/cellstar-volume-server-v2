...
from cellstar_preprocessor.tests.input_for_tests import INTERMEDIATE_ZARR_STRUCTURE_PATH_FOR_TESTS, INTERNAL_VOLUME_FOR_OMEZARR_TESTING_4_AXES, INTERNAL_VOLUME_FOR_OMEZARR_TESTING_5_AXES, INTERNAL_VOLUME_FOR_TESTING, INTERNAL_VOLUME_FOR_TESTING_XYZ_ORDER, INTERNAL_VOLUME_FOR_TESTING_ZYX_ORDER
import zarr
import pytest
    ...
    for volume_arr_resolution, volume_arr in ome_zarr_root.arrays():
        ...
        for time in range(n_of_time_groups):
            ...
            for channel in range(n_of_channel_groups):
                assert isinstance(volume_gr[volume_arr_resolution][time][channel], zarr.core.Array)
                assert volume_gr[volume_arr_resolution][time][channel].shape == volume_3d_arr_shape
                # check dtype
                assert volume_gr[volume_arr_resolution][time][channel].dtype == volume_arr.dtype

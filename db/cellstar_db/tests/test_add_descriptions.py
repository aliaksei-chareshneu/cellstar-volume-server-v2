...
async def test_add_descriptions(generate_test_data):
    ...
    annotations_metadata_before_adding: AnnotationsMetadata = await testing_db.read_annotations(
    ...
    with testing_db.edit_annotations(
        ...
        annotations_metadata_after_adding: AnnotationsMetadata = await testing_db.read_annotations(
            TEST_ENTRY_PREPROCESSOR_INPUT['source_db'],
            TEST_ENTRY_PREPROCESSOR_INPUT['entry_id']
        )

        assert len(annotations_metadata_before_adding['descriptions']) + \
            len(test_data['add_descriptions']) == \

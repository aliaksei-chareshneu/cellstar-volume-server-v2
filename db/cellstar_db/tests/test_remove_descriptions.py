...
async def test_remove_descriptions(generate_test_data):
    ...
    annotations_metadata_before_removing: AnnotationsMetadata = await testing_db.read_annotations(
    ...
    with testing_db.edit_annotations(
        ...
        annotations_metadata_after_removing: AnnotationsMetadata = await testing_db.read_annotations(
            TEST_ENTRY_PREPROCESSOR_INPUT['source_db'],
            TEST_ENTRY_PREPROCESSOR_INPUT['entry_id']
        )

        assert len(annotations_metadata_before_removing['descriptions']) - \
            len(test_data['remove_descriptions']) == \

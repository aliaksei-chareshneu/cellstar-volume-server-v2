        ...
        try:
            with self.server.run_in_thread():
                ...
                for limit in range(0, 3):
                    r = requests.get(f"{self.serverUrl()}/v2/list_entries/{limit}")
                ...
                keyword = "Drosophila"
                r = requests.get(f"{self.serverUrl()}/v2/list_entries/1000/{keyword}")
                self.assertEqual(r.status_code, 200)
                body: dict = dict(r.json())
                self.assertIsNotNone(body)
                count = 0
                ...
                # test entry keyword
                keyword = "emd"
                r = requests.get(f"{self.serverUrl()}/v2/list_entries/1/{keyword}")
                self.assertEqual(r.status_code, 200)
                body: dict = dict(r.json())
                self.assertIsNotNone(body)
                count = 0

    ...
    def test(self):
        try:
            with self.server.run_in_thread():
                ...
                segmentation_sampling_info: dict = segmentation_metadata.get("segmentation_sampling_info")
                    ...
                    for time, time_ds in resolution_ds.items():
                        for channel_id, channel_ds in time_ds.items():
                            ...
                            # data for that time and channel exists, append to it
                                ...
                                existing_data: VolumeDescriptiveStatisticsIndicators = ds_grouped[unique_key]
                                for indicator in existing_data:
                                    existing_data[indicator].append(channel_ds.get(indicator))
                ...
                for unique_key in ds_grouped:
                    ...
                    for indicator in ds:
                        self.assertEqual(
                            len(ds[indicator]), len(volume_downsamplings)
                        )                        
                    ...
                    for i in range(1, len(ds['max'])):
                        new_max = ds['max'][i]
                        self.assertAlmostEqual(previous_max, new_max, delta=abs(new_max * 0.6))
                        previous_max = new_max
                    
                    previous_std = ds['std'][0]
                    for i in range(1, len(ds['std'])):
                        new_std = ds['std'][i]
                        self.assertAlmostEqual(previous_std, new_std, delta=abs(new_std / 2))
